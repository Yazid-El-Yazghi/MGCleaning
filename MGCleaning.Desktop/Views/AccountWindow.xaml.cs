...
public partial class AccountWindow : Window
{
    ...
    private async void BlokkerenUserButton_Click(object sender, RoutedEventArgs e)
    {
        if (UsersDataGrid.SelectedItem is ApplicationUser selectedUser)
        {
            if (selectedUser.Id == _authService.HuidigeGebruiker?.Id)
            {
                MessageBox.Show("Je kunt je eigen account niet blokkeren.", "Waarschuwing", 
                    MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                if (selectedUser.LockoutEnd.HasValue && selectedUser.LockoutEnd > DateTimeOffset.UtcNow)
                {
                    // Deblokkeren
                    await _userManager.SetLockoutEndDateAsync(selectedUser, null);
                    await _userManager.SetLockoutEnabledAsync(selectedUser, false);
                    MessageBox.Show($"Gebruiker '{selectedUser.UserName}' is geactiveerd.", "Succes", 
                        MessageBoxButton.OK, MessageBoxImage.Information);
                }
                else
                {
                    // Blokkeren (permanent tot handmatig gedeblokkeerd)
                    await _userManager.SetLockoutEnabledAsync(selectedUser, true);
                    await _userManager.SetLockoutEndDateAsync(selectedUser, DateTimeOffset.MaxValue);
                    MessageBox.Show($"Gebruiker '{selectedUser.UserName}' is geblokkeerd.", "Succes", 
                        MessageBoxButton.OK, MessageBoxImage.Information);
                }
            ...
            {
                MessageBox.Show($"Fout bij blokkeren/activeren: {ex.Message}", "Fout", 
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        else
