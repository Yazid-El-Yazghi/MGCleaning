...
public partial class ProductEditDialog : Window
{
    ...
    public ProductEditDialog(ProductService productService, Product? product = null)
    {
        ...
        if (_product != null)
        {
            ...
            // Display waarde * 100 voor UI
            HoeveelheidPer100M2TextBox.Text = (_product.HoeveelheidPerM2 * 100).ToString();
    ...
    private async void OpslaanButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (!decimal.TryParse(HoeveelheidPer100M2TextBox.Text, out var per100M2) || per100M2 < 0)
            {
                MessageBox.Show("Voer een geldige hoeveelheid per 100m2 in.", "Validatie", 
                    MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            // Converteer UI waarde (per 100m2) terug naar database waarde (per m2)
            var perM2 = per100M2 / 100;
            ...
            if (_product != null)
            {
                ...
                _product.HoeveelheidPerToilet = 0; // Niet meer gebruikt
            ...
            else
            {
                ...
                var nieuwProduct = new Product
                {
                    ...
                    HoeveelheidPerToilet = 0 // Niet meer gebruikt
                };
