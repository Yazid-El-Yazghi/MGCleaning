...
public static class DbSeeder
{
    ...
    {
        // Database migraties worden automatisch toegepast via update-database
        // await context.Database.EnsureCreatedAsync(); // VERWIJDERD - gebruik migraties
    ...
    {
        // Wachtwoorden uit User Secrets
        var adminPassword = configuration["SeedPasswords:Admin"] ?? "ChangeMe123!";
        var managerPassword = configuration["SeedPasswords:Manager"] ?? "ChangeMe123!";
        var schoonmakerPassword = configuration["SeedPasswords:Schoonmaker"] ?? "ChangeMe123!";
        ...
        if (await userManager.FindByNameAsync("schoonmaker") == null)
        {
            var schoonmaker = new ApplicationUser
            {
                UserName = "schoonmaker",
                Email = "schoonmaker@mgcleaning.nl",
                Telefoonnummer = "0698765432",
                Adres = "Dorpsweg 5, Utrecht",
                EmailConfirmed = true
            };
            await userManager.CreateAsync(schoonmaker, schoonmakerPassword);
